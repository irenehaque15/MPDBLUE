...
class S3UploadsController < ApplicationController
	...
	def index
		...
		if params[:content_type] == "application/pdf"
			@doc = Document.new
			@doc.name = key
			@doc.portfolio_id = @portfolio.id
			@doc.path = 'https://s3.amazonaws.com/394_Blue/' + key
			@doc.save
		else
			@image = Image.new
			@image.name = key
			@image.portfolio_id = @portfolio.id
			@image.path = 'https://s3.amazonaws.com/394_Blue/' + key
			@image.save
		end
